...
export function Sidebar({ user }: SidebarProps) {
  ...
  return (
    <>
      ...
      {/* ── Mobile bottom nav ── */}
      <nav className="md:hidden fixed bottom-0 inset-x-0 z-40 bg-white border-t border-zinc-200 flex items-stretch">
        {navItems.map(({ href, shortLabel, icon: Icon }) => (
          ...
          >
            <Icon className={cn("h-5 w-5", pathname === href ? "text-emerald-600" : "text-zinc-400")} />
            {shortLabel}
          </Link>
        ))}
      </nav>
